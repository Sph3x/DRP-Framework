    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_customise", function()
            ...
            if(Menu.ScrollBarInt("Textures", textureScroller, textureMax, function(cb)  textureScroller = cb end)) then
                if componentScroller == 2 then
                    ...
                    player_data.ctextures[3] = textureScroller
                else
                    SetPedComponentVariation(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, paletteScroller)
                end
            end
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Textures", textureScroller, GetNumberOfPedTextureVariations(GetPlayerPed(-1), componentScroller, subComponentScroller), function(cb)  textureScroller = cb end)) then
                if componentScroller == 2 then player_data.ctextures[3] = textureScroller end
                SetPedComponentVariation(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, paletteScroller)
            end
        end)
        Menu.addOption("clothing_customise", function()
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarInt("Colours", colourScroller, 63, function(cb)  colourScroller = cb end)) then
                ...
                player_data.ocolours[componentScroller+1] = {colourType = colourType, colour = colourScroller}
            end
        end)
    else
    end
...
function save()
    ...
    for i = 0, 7 do
        player_data.pdrawables[i+1] = GetPedPropIndex(GetPlayerPed(-1), i)
        player_data.ptextures[i+1] = GetPedPropTextureIndex(GetPlayerPed(-1), i)
    end
    for i = 0, 12 do
        player_data.odrawables[i+1] = GetPedHeadOverlayValue(GetPlayerPed(-1), i)
    end

    if player_data.cdrawables[12] == 55 and GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") then player_data.cdrawables[12] = 56 SetPedComponentVariation(GetPlayerPed(-1), 11, 56, 0, 2) end
    if player_data.cdrawables[12] == 48 and GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then player_data.cdrawables[12] = 49 SetPedComponentVariation(GetPlayerPed(-1), 11, 49, 0, 2) end
...
AddEventHandler("clothes:setComponents", function()
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        for i = 0, 11 do
            if i == 0 then
                SetPedHeadBlendData(GetPlayerPed(-1), player_data.cdrawables[i+1], player_data.cdrawables[i+1], 0, player_data.cdrawables[i+1], player_data.cdrawables[i+1], 0, 0.5, 0.5, 0.0, false)
            elseif i == 2 then
                SetPedComponentVariation(GetPlayerPed(-1), i, player_data.cdrawables[i+1], 0, 1)
                SetPedHairColor(GetPlayerPed(-1), player_data.ctextures[i+1], player_data.ctextures[i+1])
            else
                SetPedComponentVariation(GetPlayerPed(-1), i, player_data.cdrawables[i+1], player_data.ctextures[i+1], player_data.cpalette[i+1])
            end
        end
        for i = 0, 7 do
        ...
        for i = 0, 12 do
            SetPedHeadOverlay(GetPlayerPed(-1), i, player_data.odrawables[i+1], player_data.oopacity[i+1])
            -- SetPedHeadOverlayColor(GetPlayerPed(-1), i, player_data.ocolours[i+1].colourType, player_data.ocolours[i+1].colour, player_data.ocolours[i+1].colour)
        end
    else
        for i = 0, 11 do
            SetPedComponentVariation(GetPlayerPed(-1), i, player_data.cdrawables[i+1], player_data.ctextures[i+1], player_data.cpalette[i+1])
