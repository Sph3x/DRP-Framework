$("document").ready(function() {
  ...
  window.addEventListener("message", function(event) {
    if (event.data.action == "open") {
      ...
      $(".overlay").hide();
      clearDocument();
      $.post("http://drp_inventory/NUIFocusOff", JSON.stringify({}));
    }
    ...
    function test() {
      ...
      test1.forEach(function(item) {
        $(".item-inner").append(
          `<div class="${item[0]} default" id="${item[0]}"><div>`
        );
        $(`#${item[0]}`).html(item[1]);
        $(`#${item[0]}`).css({
          padding: "1px",
  ...
  containers.on("drop", function(el, target, source, sibling) {
    if (source.id == "drag1" && source.id != target.id) {
      $("#drag2").each(function() {
        ...
        if (count > 1) {
          $(`.${el.id}`)[0].innerHTML =
            Number($(`.${el.id}`)[0].innerHTML) +
            Number($(`.${el.id}`)[1].innerHTML);
          $(`.${el.id}`)[1].remove();
        }
      });
    } else if (source.id == "drag2" && source.id != target.id) {
      $.post(
        "http://drp_inventory/drop",
        JSON.stringify({
  ...
  $("body").on("click", ".default", event => {
    ...
    $("#drag2")
      .children()
      .css("background-color", "rgb(31, 30, 43)");
    $("#drag2")
      .children()
  ...
  document.getElementById("button").addEventListener("click", function(event) {
    ...
    if (document.getElementsByClassName("selected")[0].id == "money") {
      ...
      $.post(
        "http://drp_inventory/dropcash",
        JSON.stringify({
          count: amount
        })
      );
    } else if (
      document.getElementsByClassName("selected")[0].id == "blackcash"
    ) {
      let amount2 = Number(document.getElementById("drop-count").value);
      $.post(
        "http://drp_inventory/dropblackcash",
        JSON.stringify({
          count: amount2
        })
      );
    } else if (
      Number(document.getElementById("drop-count").value) >
      ...
      $.post(
        "http://drp_inventory/notenoughitems",
        JSON.stringify({
      ...
      document.getElementById(
        ...
        document.getElementsByClassName("selected")[0].innerText -
        document.getElementById("drop-count").value;
