...
AddEventHandler('iens:repair', function()
	...
	if frontVehicle ~= 0 then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			local pedPos = GetEntityCoords(GetPlayerPed(PlayerId()))
			TaskStartScenarioAtPosition(ped, "WORLD_HUMAN_VEHICLE_MECHANIC", pedPos.x, pedPos.y, pedPos.z + 1.0, 180, -1, 0, 0)
			print("nigel")
		...
		notification("~r~no vehicle found giving back your Repair Kit")
		Wait(1000)
		local itemid = 15
