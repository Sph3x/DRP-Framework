...
const ATMMenu = new Vue({
    ...
    methods: {
        ...
        CloseMenu() {
            axios.post(`http://${this.ResourceName}/closeatm`, {}).then((response) => {
        ...
        DepositMoney() {
            if (this.depositFormValid) {
                this.loading = true;
                axios.post(`http://${this.ResourceName}/depositatm`, {
        ...
        WithdrawMoney() {
            if (this.withdrawFormValid) {
                this.loading = true;
                axios.post(`http://${this.ResourceName}/withdrawatm`, {
	...
	if (document.readyState == "complete") {
		window.addEventListener("message", (event) => {
            ...
            if (event.data.type == "open_atm_menu") {
                ...
                ATMMenu.UpdateMenu(event.data.status, event.data.message, event.data.balance, event.data.cash);
