$('document').ready(function () {
    ...
    containers.on('drop', function (el, target, source, sibling) {
        if (source.id == 'drag1' && source.id != target.id) {
            ...
            $.post('http://drp_inventory/drop', JSON.stringify({
    ...
    document.getElementById('button-use').addEventListener('click', function (event) {
        ...
        $.post('http://drp_inventory/use', JSON.stringify({
            item: itemSelected
        }));
    })
    document.getElementById("button").addEventListener("click", function (event) {
        ...
        if (document.getElementsByClassName('selected')[0].id == 'money') {
            ...
            $.post('http://drp_inventory/dropcash', JSON.stringify({
                count: amount
            }));
            ...
            $.post('http://drp_inventory/dropblackcash', JSON.stringify({
                count: amount2
            }));
            ...
            $.post('http://drp_inventory/notenoughitems', JSON.stringify({
