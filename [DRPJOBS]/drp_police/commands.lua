RegisterCommand("addcop", function(source, args, raw)
    ...
    if myJob.job == "POLICE" or myJob.job == "SHERIFF" or myJob.job == "STATE" then
            ...
            if myJob.otherJobData.rank >= DRPPoliceJob.RankAllowedToAddCop then
                ...
                exports["externalsql"]:DBAsyncQuery({
                    ...
                    if json.encode(isPoliceOfficer["data"]) == "[]" then
                        ...
                        TriggerClientEvent("DRP_Core:Info", src, "Government", "This Person is already a Cop, do not need to add them twice", 5500, false, "leftCenter")
                    end
                end)
            else
        ...
        TriggerClientEvent("DRP_Core:Error", src, "Government", "You are not an Employee of the Police Force", 5500, false, "leftCenter")
    end
end, false)
